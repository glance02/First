...
我实验采用 Zenodo 平台公开数据集“LIGHT dataset sample for solar smart home pilot - 201801”中的一个真实光伏站点样本，选取 `ID00002` 作为研究对象。原始数据为 1 分钟粒度的功率序列，为了降低“一分钟后几乎等于当前时刻”这一极强基线带来的影响，实验将其重采样为 15 分钟粒度，并保留单站点功率序列作为核心输入。配套脚本位于 `experiments/pv_forecast.py`，运行后会自动输出误差表、预测结果表、曲线图和可直接贴回正文的实验摘要。
...
本实验采用以下配置。首先，将原始 1 分钟数据重采样为 15 分钟数据；其次，加入基于时间戳构造的周期性时间特征，包括昼夜周期和周内周期；再次，使用过去 20 个时间步，也就是过去 5 小时的数据，预测未来 2 个时间步，也就是未来 30 分钟的光伏功率。数据经过时间排序后，不再默认过滤夜间零出力区段，而是保留完整的昼夜功率变化，并按 70%/15%/15% 切分为训练集、验证集和测试集。训练阶段采用验证集 RMSE 做早停，并对窗口长度、隐藏层宽度和 batch size 做了简单调参，评价指标固定为 MAE、RMSE 和 MAPE。这样的设置更接近电站全天功率预测的真实场景，也能避免图中因为白天片段被抽离而出现过长的时间断裂。

### 4.3 实验结果

基于上述真实数据和配置，我得到了一些简单的实验结果。
...
从表 3 可以看出，在“15 分钟重采样 + 时间特征 + 30 分钟 ahead 预测 + 保留完整昼夜序列”的设置下，Persistence 在 RMSE 指标上仍然表现最好，而 LSTM 的误差略高于 Persistence、优于 MLP。这说明当夜间零功率区段被完整保留时，简单的延续性基线会因为大量平稳样本而变得更强，深度学习模型并不一定会自动取得更优结果。

需要说明的是，Persistence 的 MAPE 数值仍然偏大，主要是因为光伏功率在夜间、日出和日落等低功率时段接近零，百分比误差容易被放大。因此，在解释结果时应更关注 RMSE 和 MAE 这类对工程应用更稳定的指标。综合来看，本实验支持这样一个判断：如果按全天连续功率预测来评价，强基线模型往往比预想中更难超越；深度学习模型的价值更多体现在对白天波动段、峰值变化和复杂时序关系的拟合，而不是在所有评价口径下都天然占优。

[图2：测试集整段时间上的真实值与各模型预测曲线，建议插入 `artifacts/pv_experiment_light/prediction_curve.png`]
![](artifacts/pv_experiment_light/prediction_curve.png)
