...
随着风电、光伏等新能源装机容量持续增长，电力系统正在从以稳定电源为主的结构转向高波动、多主体、强不确定性的运行形态。新能源出力预测误差会直接影响备用容量配置、储能调度、购售电计划以及微电网经济运行，因此预测能力已经成为智慧能源系统中的关键基础能力。深度学习由于具备较强的非线性建模能力和时序模式提取能力，近年来被广泛用于负荷预测、风电功率预测、光伏功率预测和微电网能量管理等场景。在这份报告中，我围绕“深度学习如何支撑智慧能源”这一问题展开分析，重点聚焦光伏功率预测，并结合风电预测与微电网调度案例，梳理深度学习在智慧能源中的主要应用路径、代表性模型和工程价值。除了文献综述之外，我还基于公开的 LIGHT 光伏数据集做了一个小实验：将 1 分钟功率数据重采样为 15 分钟数据，加入时间周期特征，并比较 Persistence、MLP 和 LSTM 在 30 分钟 ahead 预测任务中的表现。实验结果表明，LSTM 在 MAE 和 MAPE 指标上优于 MLP 和 Persistence，RMSE 与强基线 Persistence 接近，说明在更合理的时间粒度和特征设计下，深度学习模型能够体现出一定优势。综合来看，LSTM、GRU、CNN-LSTM 等模型在新能源预测中具有较好应用前景，但深度学习在智慧能源中的落地仍面临数据质量、跨场景泛化、可解释性和部署成本等问题。未来研究应更多关注机理与数据融合、预测与优化一体化以及多模态数据协同建模等方向。
...
本实验采用以下配置。首先，将原始 1 分钟数据重采样为 15 分钟数据；其次，加入基于时间戳构造的周期性时间特征，包括昼夜周期和周内周期；再次，使用过去 16 个时间步，也就是过去 4 小时的数据，预测未来 2 个时间步，也就是未来 30 分钟的光伏功率。数据经过时间排序后，默认过滤长时间连续的夜间零出力区段，并按 70%/15%/15% 切分为训练集、验证集和测试集。训练阶段采用验证集 RMSE 做早停，评价指标固定为 MAE、RMSE 和 MAPE。这样的设置比直接做“下一分钟预测”更接近实际能源调度对短时预测的需求，同时也比 1 小时 ahead 的任务更容易形成清晰、平滑的结果图。
...
| MLP | 0.3688 | 0.5380 | 43.73 |
| LSTM | 0.3066 | 0.5159 | 33.02 |

从表 3 可以看出，在“15 分钟重采样 + 时间特征 + 30 分钟 ahead 预测”的设置下，Persistence 依然是一个非常强的基线，在 RMSE 指标上略优于 LSTM；但与此同时，LSTM 在 MAE 和 MAPE 上优于 Persistence 和 MLP，说明它在整体平均偏差控制方面表现更稳定。这个结果说明，对于短时光伏预测任务而言，深度学习模型未必会在所有指标上全面领先，但在更复杂的时间依赖建模和平均误差控制上仍然具有一定优势。

需要说明的是，Persistence 的 MAPE 数值偏大，主要是因为光伏功率在日出、日落等低功率时段接近零，百分比误差会被放大。因此，在解释结果时应更关注 RMSE 和 MAE 这类对工程应用更稳定的指标。综合来看，本实验支持这样一个判断：在更合理的时间粒度和预测步长设定下，深度学习模型能够提供比简单前馈网络更好的结果，并在部分关键指标上接近或优于强基线。
