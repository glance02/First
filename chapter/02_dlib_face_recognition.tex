...
\subsection{分析目的与章节边界}

上一章分析了 \mintinline{text}|face-api.js| 如何在浏览器端加载预训练模型并完成推理。本章换一个角度，分析 dlib 官方示例 \path{code/dlib/examples/dnn_metric_learning_on_images_ex.cpp}，重点说明人脸识别模型背后的深度度量学习思想。两章之间的关系不是“同一项目的前后端实现”，而是“推理应用”和“训练思想”的对应：浏览器页面展示模型如何被调用，dlib 示例展示类似的人脸描述子模型可以如何训练。

本章主体不再重复前端页面的上传、Canvas 绘制和结果渲染流程，而是围绕 C++ 示例中的训练数据组织、mini-batch 采样、网络结构、损失函数、训练器、多线程加载、模型保存和阈值验证展开。只在开头和结尾保留少量对照，帮助理解为什么浏览器项目中的 \mintinline{javascript}|FaceMatcher| 和注册样本并不等同于重新训练模型。
...
\begin{figure}[htbp]
  ...
  此处放置 dlib 示例源码截图。\\
  建议文件名：\texttt{\detokenize{img/dlib_metric_learning_source.png}}。\\
  截图内容建议包含 \texttt{\detokenize{dnn_metric_learning_on_images_ex.cpp}} 文件名和 \texttt{\detokenize{loss_metric}}、\texttt{\detokenize{net_type}} 等关键代码。}}
  \caption{dlib 度量学习示例源码占位图}
  \label{fig:dlib_source_placeholder}
\end{figure}
...
\begin{figure}[htbp]
  ...
  此处放置训练数据目录截图。\\
  建议文件名：\texttt{\detokenize{img/dlib_training_dataset_tree.png}}。\\
  截图内容建议展示顶层目录、多个身份子目录和每个身份下的多张图片。}}
  \caption{dlib 训练数据目录组织占位图}
  \label{fig:dlib_dataset_tree_placeholder}
\end{figure}
...
\subsubsection{为什么度量学习依赖 batch 组成}
...
\begin{figure}[htbp]
  \centering
  \fbox{\parbox[c][0.24\textheight][c]{0.84\textwidth}{\centering
  此处放置 mini-batch 采样示意图。\\
  建议文件名：\texttt{\detokenize{img/dlib_minibatch_sampling.png}}。\\
  建议内容：5 个身份，每个身份 5 张图，标出同身份正样本对和不同身份负样本对。}}
  \caption{深度度量学习 mini-batch 采样占位图}
  \label{fig:dlib_minibatch_placeholder}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \fbox{\parbox[c][0.24\textheight][c]{0.84\textwidth}{\centering
  此处放置网络结构示意图。\\
  建议文件名：\texttt{\detokenize{img/dlib_resnet_descriptor_network.png}}。\\
  建议内容：输入人脸图像、卷积与残差层、全局平均池化、128 维输出描述子。}}
  \caption{dlib 人脸描述子网络结构占位图}
  \label{fig:dlib_network_placeholder}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \fbox{\parbox[c][0.24\textheight][c]{0.84\textwidth}{\centering
  此处放置嵌入空间距离示意图。\\
  建议文件名：\texttt{\detokenize{img/face_embedding_space.png}}。\\
  建议内容：二维简化坐标中，同一身份样本聚成一簇，不同身份簇之间距离较远，并标出距离阈值。}}
  \caption{人脸嵌入空间与距离阈值占位图}
  \label{fig:embedding_space_placeholder}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \fbox{\parbox[c][0.24\textheight][c]{0.84\textwidth}{\centering
  此处放置 dlib 主函数训练流程图。\\
  建议文件名：\texttt{\detokenize{img/dlib_main_training_flow.png}}。\\
  建议内容：参数检查、读取数据、初始化网络、启动加载线程、训练循环、保存模型、验证阈值。}}
  \caption{dlib 示例主函数训练流程占位图}
  \label{fig:dlib_main_flow_placeholder}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \fbox{\parbox[c][0.22\textheight][c]{0.84\textwidth}{\centering
  此处放置训练日志或多线程加载示意截图。\\
  建议文件名：\texttt{\detokenize{img/dlib_training_log.png}}。\\
  截图内容建议包含训练日志输出、学习率变化或数据加载队列示意。}}
  \caption{dlib 训练日志与数据加载占位图}
  \label{fig:dlib_training_log_placeholder}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \fbox{\parbox[c][0.22\textheight][c]{0.84\textwidth}{\centering
  此处放置阈值评估曲线示意图。\\
  建议文件名：\texttt{\detokenize{img/face_threshold_tradeoff.png}}。\\
  建议内容：横轴为距离阈值，纵轴为误识率和拒识率，展示阈值变化带来的取舍。}}
  \caption{距离阈值选择与错误类型取舍占位图}
  \label{fig:threshold_tradeoff_placeholder}
\end{figure}
...
因此，两章合起来可以形成完整链路：第 2 章说明模型如何通过深度度量学习获得特征空间，第 1 章说明前端页面如何加载预训练模型并在浏览器中执行推理。为了保持章节边界，本章只在这里做对照，不再展开前端页面的 DOM、Canvas 和摄像头流程。
...
从学习角度看，dlib 示例最值得借鉴的是方法论：用目录组织身份数据，用 mini-batch 同时提供正负样本关系，用 ResNet 提取图像特征，用 \mintinline{cpp}|loss_metric| 约束嵌入距离，用阈值完成训练后验证。这些步骤构成了人脸描述子模型训练的基本骨架。理解这个骨架后，再看浏览器端的预训练模型推理，就能更清楚地区分“模型已经学到的能力”和“页面临时完成的匹配逻辑”。
...
训练完成后，模型被序列化保存，并可以通过描述子距离和阈值进行验证。这种“描述子提取 + 距离判断”的思想解释了人脸识别系统为什么不必把每个人都作为固定分类类别，也解释了浏览器实验中注册样本和 \mintinline{javascript}|FaceMatcher| 的作用。与上一章结合后，报告能够从训练思想和前端推理两个层面说明人脸分析系统的整体工作方式。
