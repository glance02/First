...
我在这个实验中主要对比了三类模型：Persistence、MLP 和 LSTM。一下简要介绍一下每一个模型。
...
其实目前在智慧能源领域，除了 LSTM 以外，还有很多其他类型的深度学习模型也被广泛应用，例如 GRU、CNN-LSTM、TCN 和 Transformer 等，尤其是Transformer 。但是在这个实验中，我选择了 LSTM 作为代表性的深度学习模型，主要是因为它在时序预测领域的经典地位和广泛应用，同时也为了保持实验的简洁性和可复现性。未来如果有机会，我也计划在这个基础上进一步引入其他类型的模型进行对比分析，以更全面地展示深度学习在光伏功率预测中的潜力和适用性。
